...
namespace Mocha_Finance.Model
{
    public class MyFavouriteModelContext
    {
        ...
        public MyFavourite GetSingleMyFavouriteByMemberID(int memberId)
        {
            MyFavourite fav;
            try
            {
                StockDBContext sContext = new StockDBContext();
                fav = sContext.MyFavouries.FirstOrDefault(m => m.MemberID == memberId);
            }
            catch (Exception ex)
            {
        ...
        {
            MyFavourite mFav = new MyFavourite(memberId, favName);
            try
            {
                StockDBContext sContext = new StockDBContext();
